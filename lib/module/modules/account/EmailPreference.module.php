	...
	public function handleAction($action, $cleaned) {
		switch ($action) {	
			case "update":
				...
				$sys -> sendEmail($this -> user, "Hi " . $user -> getFormatUsername() . ",<br>We have received a request to change the registered email address for your WildScape Pk account.<br>To start this change, please follow the link below or copy and paste it into your browser:<br> " . VALIDATE_LINK . Registry::get("sys")->getValidationManager()->newValidation($user->getUsername(), 2) . "<br>Kind regards,<br> WildScape Pk Account Support");
				echo "SUCCESS";
				break;
		}
	}
