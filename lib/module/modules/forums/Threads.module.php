...
class Threads extends Module {
	...
	private function createThread($cleaned) {
		...
		$date = new DateTime('now');
                ...
                $useruid = $user -> getUid();
		$statement = $db->prepare("INSERT INTO web_forums.threads (board_id, title, starter_uid, date) VALUES(?,?,?,?)");
		...
		$statement = $db->prepare("INSERT INTO web_forums.posts (thread_id, board_id, poster, content, date, first_post) VALUES(?,?,?,?,?,?)");
	...
	private function submitPost($cleaned) {
		...
		if (!isset($cleaned['pid']) || $cleaned['pid'] == 0) {
			...
			$userUid =  $user -> getUid();
			$statement = $db->prepare("INSERT INTO web_forums.posts (thread_id, board_id, poster, content, date) VALUES(?,?,?,?,?)");
			...
			$statement = $db -> prepare("UPDATE web_forums.posts SET content=?,lastEdit=? WHERE post_id=?");
