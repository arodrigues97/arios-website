...
class Register extends Action {
	...
	public function checkRegister($email, $username, $password, $password2, $bot) {
		...
		if ($instanced->exists()) {
			Registry::get("sys") -> sendEmail($instanced, "Hi " . $instanced -> getFormatUsername() . ",<br>Please visit the following link to update your email address! </br> " . VALIDATE_LINK . Registry::get("sys")->getValidationManager()->newValidation($instanced->getUsername(), 1, $email) . "<br>Kind Regards,<br>WildScape Pk Account Support", $email);
		}
		return "SUCCESS";
	}
