	...
	public function handle($cleaned) {
		...
		if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
			if ($emailUpdate) {
				...
				$sys -> sendEmail($user, "Hi " . $user -> getFormatUsername() . ",<br>Please visit the following link to update your email address! </br> " . VALIDATE_LINK . Registry::get("sys")->getValidationManager()->newValidation($user->getUsername(), 1, $email) . "<br> Kind Regards,<br>WildScape Pk Account Support", $email);
			...
			if ($statement->execute(array($email))) {
				if ($statement->rowCount() == 0 || $user->getEmail() == $email) {
					...
					if ($user->write()) {
						$sys -> sendEmail($user, "Hi " . $user -> getFormatUsername() . ",<br>Please visit the following link to activate your WildScape Pk account: </br> " . VALIDATE_LINK .Registry::get("sys")->getValidationManager()->newValidation($user->getUsername(), 1). "<br>Kind Regards,<br>WildScape Pk Account Support");
						echo "Success! Verification link sent to ".$email;
					}
				} else {
