...
$(document).ready(function() {
	...
	$(document).delegate(".editor > .core > .submit", "click", function() {
		$.post("/lib/sys/ActionHandler.php?action=ModuleAction&name=Threads&modAction=" + actionType, {html: editor.getData(), tid: <?php echo $_GET['id']?>, pid: pid, bid: <?php echo $_GET['board_id'];?>, page: <?php echo $_GET['page']?>, replyId:replyId}, function(data) {
		...
		if ($(this).hasClass("edit")) {
			...
			pid = 0;
			  ...
			  if (window.confirm("Are you sure?")) {
				  $.post("/lib/sys/ActionHandler.php?action=ModuleAction&name=Threads&modAction=delete", {tid: <?php echo $_GET['id']?>, pid: pid, bid: <?php echo $_GET['board_id'];?>, page: <?php echo $_GET['page']?>}, function(data) {
						if (data == "SUCCESS") {
